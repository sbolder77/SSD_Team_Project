...
class LoggingDetails:
    ...
    def write_user_log(self, logtype, level, message, user):
    ...
    def write_orders_log(self, logtype, level, message, user):
    ...
    def write_item_log(self, logtype, level, message, user):
    ...
    def download_log(self, user, logtype):
        if user == 'A':
            if logtype == 'SYSTEM':
                shutil.copyfile(syslog, './Downloads/' + syslog + DT_STR + '.csv')
